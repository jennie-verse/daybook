...
- Today는 그날 Today에 추가된 결과만 보여줍니다(Someday로 미룬 항목은 제외). 종류에 따라 다르게 표시합니다: Task는 완료 `[x]` / 미완료 `[-] ~~할 일~~`(그날 안 된 일은 취소로 표시), Note는 `- 내용`, Event는 예정 시각(`HH:MM 내용`)입니다. 종류 정보가 없는 옛 기록은 지금처럼 Task로 표시됩니다.
...
- **Markdown escape**: `reading-1` 같은 제목에서 불필요한 백슬래시(`reading\-1`)가 보이지 않고, Preview는 escape 문자도 원래 글자로 표시합니다.

## Settings — 글자 크기, 삭제 확인, 보존된 충돌
